...
export const Barcode = ({ fetchDetailsHandler }: InputProps) => {
	...
	useEffect(() => {
		...
		const enableCamera = async () => {
			try {
				const stream =
					await navigator.mediaDevices.getUserMedia(videoCaptureOptions);
		...
		return () => {
			if (videoTrackRef.current) {
				...
				videoTrackRef.current
					.applyConstraints({
						advanced: [
							{ torch: false } as MediaTrackConstraintSet & { torch: boolean },
						],
					})
					.catch(console.error);
		...
		if (videoTrack) {
			try {
				await videoTrack.applyConstraints({
					advanced: [
						{ torch: !isFlashOn } as MediaTrackConstraintSet & {
							torch: boolean;
						},
					],
				});
